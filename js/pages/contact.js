console.log('📞 CONTACT.JS LOADED - Contact page specific functionality');

// Import page-specific functionality for contact.html
...
const initContactPage = async () => {
  ...
  // Initialize contact form (needed for #contactForm)
  const contactForm = new ContactForm('#contactForm');

  // Make contact form available globally for transitions
  ...
  // Configure canvas based on template (contact.html uses "hybrid" template)
  ...
  window.pageTransitionHandlers.push(() => {
    console.log('Contact page transition handler');
    // Contact form may need reinitialization after transitions
    if (window.contactForm) {
      // ContactForm handles its own reinitialization internally
    }
  });
...
// Hybrid mode functionality (for contact.html)
...
initContactPage().catch(console.error);