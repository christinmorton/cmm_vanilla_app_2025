console.log('📅 CONSULTATION-BOOKING.JS LOADED - Consultation booking page specific functionality');

// Import page-specific functionality for consultation-booking.html
...
// Initialize consultation booking page functionality
const initConsultationBookingPage = async () => {
  console.log('Initializing consultation booking page functionality...');

  // Wait for core components
  const { preloader } = await waitForCore();

  // Initialize consultation booking functionality (existing module)
  // ConsultationBooking module handles its own initialization
  // Note: ConsultationBooking.js is already imported and will initialize automatically
  ...
  // Configure canvas based on template (consultation-booking.html uses "hybrid" template)
  ...
  window.pageTransitionHandlers.push(() => {
    console.log('Consultation booking page transition handler');
    // ConsultationBooking module handles its own state management
  });

  console.log('✅ Consultation booking page functionality initialized');
};

// Hybrid mode functionality (for consultation-booking.html)
...
initConsultationBookingPage().catch(console.error);