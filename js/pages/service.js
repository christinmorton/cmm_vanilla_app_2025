console.log('💼 SERVICE.JS LOADED - Service page specific functionality');

// Import page-specific functionality for service.html
...
const initServicePage = async () => {
  console.log('Initializing service page functionality...');
  ...
  // Configure canvas based on template (service.html uses "hybrid" template)
  ...
  // Register page transition handler (minimal for service page)
  window.pageTransitionHandlers = window.pageTransitionHandlers || [];
  window.pageTransitionHandlers.push(() => {
    console.log('Service page transition handler');
    // Service page has minimal transition requirements
  });

  console.log('✅ Service page functionality initialized');
};

// Hybrid mode functionality (for service.html)
...
initServicePage().catch(console.error);