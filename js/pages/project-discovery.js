console.log('🔍 PROJECT-DISCOVERY.JS LOADED - Project discovery page specific functionality');

// Import page-specific functionality for project-discovery.html
import DesignGridWindow from '../modules/DesignGridTypes/index.js';
import ProjectDiscovery from '../modules/ProjectDiscovery.js';
...
// Initialize project discovery page functionality
const initProjectDiscoveryPage = async () => {
  console.log('Initializing project discovery page functionality...');

  // Wait for core components
  const { preloader } = await waitForCore();

  // Initialize project discovery functionality (existing module)
  // ProjectDiscovery module handles its own initialization
  // Note: ProjectDiscovery.js is already imported and will initialize automatically
  ...
  // Configure canvas based on template (project-discovery.html uses "hybrid" template)
  ...
  window.pageTransitionHandlers.push(() => {
    console.log('Project discovery page transition handler');
    // ProjectDiscovery module handles its own state management
  });

  console.log('✅ Project discovery page functionality initialized');
};

// Hybrid mode functionality (for project-discovery.html)
...
initProjectDiscoveryPage().catch(console.error);